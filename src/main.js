...
function initServiceSlider() {
  ...
  // Fungsi untuk menghitung lebar scroll yang pas berdasarkan kartu pertama
  const getScrollAmount = () => {
    const firstCard = carousel.querySelector(".service-card");
    if (!firstCard) return 320;
    return firstCard.offsetWidth + 24; // 24 adalah gap antar kartu
  };
...
function initCardInteractions() {
  ...
  const triggerClickEffect = (element, callback = null) => {
    gsap.fromTo(element, { scale: 1 }, { scale: 0.92, duration: 0.1, ease: "power2.in", onComplete: () => {
      gsap.to(element, { scale: 1.05, duration: 0.15, ease: "power2.out", onComplete: () => {
  ...
  serviceCards.forEach(card => { card.addEventListener("click", () => { triggerClickEffect(card); }); });
}
...
if (window.location.pathname.includes("project-detail.html")) {
  ...
  if (project) {
    ...
    gsap.set(".back-btn, .detail-role, .detail-title, .detail-desc, .detail-meta, .detail-actions, .detail-image-wrap, .detail-content, .detail-sidebar", { opacity: 0, y: 20 });
    gsap.timeline({ defaults: { duration: 0.8, ease: "power3.out" } })
      ...
      .to(".detail-meta", { opacity: 1, y: 0, stagger: 0.1 }, "-=0.2")
  ...
  } else { document.getElementById("d-title").textContent = "Proyek tidak ditemukan."; }
} else {
  const loader = document.getElementById("loading-screen");
  if (loader) {
    gsap.to(loader, { opacity: 0, duration: 1, delay: 1, onComplete: () => { loader.style.display = "none"; animateEntranceHome(); setupScrollAnimations(); } });
  } else { animateEntranceHome(); setupScrollAnimations(); }
}
...
if (floatBtn) {
  ...
  floatBtn.addEventListener("click", () => {
    ...
    if (isLightOn) {
      floatBtn.style.color = "#fbbf24"; floatBtn.style.filter = "drop-shadow(0 0 10px #fbbf24)";
      document.body.classList.add("light-mode");
    } else {
      floatBtn.style.color = "white"; floatBtn.style.filter = "none";
